...
switch (true) do
{
	...
	case (_veh isKindOf "Tank") :
	{
		switch(true) do
		{
			case (damage _veh > 0.15) : {_price = 1950;};
			case (damage _veh > 0.06) : {_price = 1650;};
			case (damage _veh > 0.03) : {_price = 1350;};
			default {_price = 25;};
		};
		
		switch (true) do
		{
			case (fuel _veh < 0.2) : {_price = _price + 1200;};
			case (fuel _veh < 0.5) : {_price = _price + 1150;};
			case (fuel _veh < 0.8) : {_price = _price + 1100;};
			default { _price = _price + 50;};
		};
	};
};
