	...
	if (count AllowedVehiclesList == 0) then {
		...
		if ((count _position) == 2) then { 
			...
			for "_x" from 1 to _num do {
				...
				//diag_log("DEBUG: spawned loot inside vehicle " + str(_itemType));
			};
...
//Skalisty
execVM "\z\addons\dayz_server\CustomBuildings\skalisty.sqf";
...
DefaultWeapons = ["ItemMap","ItemFlashlight","ItemToolbox","ItemRadio","ItemKnife","glock17_EP1"]; 
