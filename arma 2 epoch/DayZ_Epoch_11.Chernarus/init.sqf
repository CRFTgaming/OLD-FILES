...
dayz_minpos = -1; 
dayz_maxpos = 16000;
dayz_sellDistance_vehicle = 20; // default 10
dayz_sellDistance_boat = 30;    // default 30
dayz_sellDistance_air = 50;     // default 40
...
if (!isDedicated) then {
	...
	// Player Radio Msg - Random Bombings
    //	[] execVM "scripts/player_radioMsg.sqf";// to be fixed sometime
...
if(SnowScript)then{
	snowchance =5; //default is 5%. Chance to snow on server start/restart
	if (isServer) then {
