...
sand_shieldON = {
	private ["_txt"];
	_txt = "You're A Regular Rick Grimes Now.";
	systemChat ("Zombie ninja: "+str _txt+"");
				...
				if(!isNull player) then {
				  if((speed player <= 1) && (_canClean || _pondNear || _wellNear) && _canDo) then {
				      if (s_player_cleanguts < 0) then {
				          s_player_cleanguts = player addaction["<t color=""#ff4444"">" + "Clean Guts" + "</t>","scripts\walkamongstthedead\usewatersupply.sqf","",5,false,true,"",""];
				      };
				  } else {
