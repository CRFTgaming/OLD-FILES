...
_nul = [] execVM "AEG\Functions\AEG_fnc_actions.sqf";
...
{ 
    ...
    if (_action) then
    {
        ...
        {
            _trip = (position _logic nearestObject _object) addAction ["Trip Circuit Breaker", "AEG\Scripts\AEG_actions.sqf", _name, 6, true, true, "", (format ["((AEG_manual) && (AEG_out_%1) && ((_this distance _target) < 7))", _name])];    
            _close = (position _logic nearestObject _object) addAction ["Close Circuit Breaker", "AEG\Scripts\AEG_actions.sqf", _name, 6, true, true, "", (format ["((AEG_manual) && (!AEG_out_%1) && ((_this distance _target) < 7))", _name])];
        }
        else
        {
    ...
    switch (_effect) do
    {
        case "XFMR":
        {
            _nul = ["XFMR", _name] execVM "AEG\Scripts\AEG_sounds.sqf";
        };

        case "PLANT":
        {
            _nul = ["PLANT", _name] execVM "AEG\Scripts\AEG_sounds.sqf";
        };

        case "SMOKE":
        {
            _nul = [_name] execVM "AEG\Scripts\AEG_smoke.sqf";
        };
    };
...
// Add a respawn event handler to ensure the simulation is consistent for respawning players.  The EH executes a script for that purpose.
 player addEventHandler ["Respawn", {_nul = [] execVM "AEG\Scripts\AEG_respawn.sqf"}];
